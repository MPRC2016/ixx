...
Boolean Operation::generate_stub(Generator* g) {
    ...
    long rdesc;
    if (oneway_) {
	rdesc = g->symbol_table()->oneway_type()->typename_()->kind();
    } else {
...
long Operation::type_desc(Generator* g, Expr* e) {
	...
	rdesc = g->symbol_table()->long_type()->typename_()->kind();
	break;
    case Symbol::sym_string:
	rdesc = g->symbol_table()->string_type()->typename_()->kind();
	break;
    case Symbol::sym_typedef:
	rdesc = s->typename_()->kind();
	break;
...
void Operation::generate_return_value(Generator* g, Expr* t) {
	...
	break;
    case Symbol::sym_typedef:
	g->emit("_arg[0].u_%I", s->typename_()->str());
	break;
    default:
	g->emit("_result");
	break;
...
void Operation::generate_receive_asg(Generator* g, Expr* type) {
    ...
    switch (s->tag()) {
    case Symbol::sym_string:
	g->emit("string = ", s->typename_()->str());
	break;
    case Symbol::sym_typedef:
	g->emit("%I = ", s->typename_()->str());
	break;
    case Symbol::sym_enum:
	g->emit("long = ");
	break;
